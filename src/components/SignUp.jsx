...
function SignUp() {
  ...
  const handleSubmit = e => {
    ...
    // Simulate sign up success, pass name to navbar
    window.dispatchEvent(new CustomEvent("user-login", { detail: form.name }));
    navigate("/");
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#1a2233] to-[#232b3e] pt-24">
      <form onSubmit={handleSubmit} className="bg-white/10 backdrop-blur rounded-2xl p-8 shadow-lg w-full max-w-md border border-white/20">
        ...
        <button type="submit" className="w-full bg-blue-500 text-white font-bold py-2 rounded-lg hover:bg-blue-600 transition">Sign Up</button>
